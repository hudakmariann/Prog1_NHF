...
HSV** RGBtoHSV(const ImageParams*  imgdata, const PixelData**  matrix){
    double min, max, diff;
    HSV** HSVmatrix;
    double r, g, b; //segedvaltozok, a kod atlathatosaga, rovidites miatt vannak, amugy redundans
    ...
    for (int i = 0; i < imgdata->h; i++){
        for (int j = 0; j < imgdata->w; j++){
            ...
            if (HSVmatrix[i][j].h < 0)
                HSVmatrix[i][j].h=0;  //ha negativ, teljesen elromlik a kep, csikos lesz. Ha pozitiv, akkor elromlanak nehol a szinek

            HSVmatrix[i][j].h /= 6;
            //printf ("h[%d][%d]= %.2f ", i, j, HSVmatrix[i][j].h );
