...
void blurimage(ImageParams const *imgdata, PixelData  **matrix){
    ...
    printf("Please provide the factor of blur between 3 and 30.\n");
    scanf("%d", &filtersize);
    if (filtersize < 3){
        filtersize = 3;
        printf("Warning: the requested factor is out of range. The factor is set to 3.\n ");
    }
    else if (filtersize > 30){
        filtersize = 31;
        printf("Warning: the requested factor is out of range. The factor is set to 30.\n ");
    }
    if (filtersize%2 == 0)
        filtersize++;
